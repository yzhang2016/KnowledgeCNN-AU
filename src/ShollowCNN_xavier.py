...
import ShollowCNN_AUModel_weak as ShollowCNN_AUModel


IMG_SIZE = bp4d_input.IMAGE_SIZE
NUM_CLASS = ShollowCNN_AUModel.NUM_CLASSES

W_DECAY = 1e-5 *0 /ShollowCNN_AUModel.BATCH_SIZE   # fully connected layer
...
class ShollowCNN:
    ...
    def inference(self,rgb,train_mode=None,is_build=True):
        ...
        filter_size = 5
        self.conv1 = self.conv_layer_relu(bgr,filter_size,1,3,32,'conv1',
                                          trainable=True,is_build=is_build)
        self.pool1 = self.max_pool(self.conv1,3,2,'pool1')    
        
        self.conv2 = self.conv_layer_relu(self.pool1,filter_size,1,32,64,'conv2',
                                          trainable=True,is_build=is_build)
        self.pool2 = self.max_pool(self.conv2,3,2,'pool2')
        
        self.conv3 = self.conv_layer_relu(self.pool2,filter_size,1,64,128,'conv3',
                                          trainable=True,is_build=is_build)
...
#                                      trainable=True,w_decay=W_DECAY,is_build=is_build)
        ...
        self.fc4 = self.fc_layer_relu(self.pool3, 2048, 128,'fc4',
                                      trainable=True,w_decay=W_DECAY,is_build=is_build)
    ...
    def get_conv_var(self,filter_size,in_chs,out_chs,name,
                     trainable,w_decay,is_build):
        initial_value = tf.truncated_normal([filter_size,filter_size,in_chs,out_chs],mean=.0,stddev=0.001)
        filters = self.get_var(initial_value,[filter_size,filter_size,in_chs,out_chs], name,0, name+'_filters',
                               trainable,w_decay,is_build)
        
        initial_value = tf.truncated_normal([out_chs],mean=0.01,stddev=.0)
        biases = self.get_var(initial_value,[out_chs], name,1,name+'_biases',
                              trainable,w_decay=None,is_build=is_build,is_biases=True)
        
        return filters, biases
        
    def get_fc_var(self,in_size,out_size,name,
                   trainable,w_decay,is_build):
        initial_value = tf.truncated_normal([in_size,out_size],mean=.0,stddev=0.001)
        weights = self.get_var(initial_value,[in_size,out_size], name,0,name+'_weights',
                               trainable,w_decay,is_build)
        
        initial_value = tf.truncated_normal([out_size],mean=0.0,stddev=.0)
        biases = self.get_var(initial_value,[out_size],name,1,name+'_biases',
                              trainable,w_decay=None,is_build=is_build,is_biases=True)
        
        return weights, biases
    
    def get_var(self,initial_value,shape,name,idx,var_name,trainable,w_decay,is_build,is_biases=False):
        if is_build:
            if self.data_dict is not None and name in self.data_dict:
                ...
                if trainable:
                    if is_biases:
                        ...
                        Init_xvaier = tf.contrib.layers.xavier_initializer(uniform=False, seed=None, dtype=tf.float32)
                        var = tf.get_variable(name=var_name, shape=shape, initializer=Init_xvaier)
                        if w_decay is not None:
                            weight_decay = tf.multiply(tf.nn.l2_loss(var),w_decay,name='weight_loss')
                            tf.add_to_collection('losses',weight_decay)
                else:
                    raise ValueError('Not assigned constant..')
